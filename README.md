...
![EV Fleet Operations Dashboard](docs/dashboard.png)
...
| Data Format | JSON (Bronze) to Parquet Snappy (Silver) to JSON snapshots (Gold) |
...
|---|---|---|
| 1 | DO_NOT_CHARGE | battery_temp >= 45C — thermal safety override |
...
Containerised Lambda (Docker on ECR) queries 30 days of Silver telemetry via two-level Athena aggregation, fits Prophet with CmdStan C++ backend, produces 24-hour-ahead forecast with 80% confidence intervals. Grid breach confirmed at 252.9kW vs 150kW physical limit.

Why Docker: CmdStan C++ backend exceeds 250MB Lambda zip limit. Container images support up to 10GB.

Training data: Naive SUM of charger_kw inflates 50-500x. Correct approach: AVG per vehicle per hour (inner), SUM across vehicles (outer). Max realistic output: 10 vehicles x 50kW = 500kW.
...
Authentication via OIDC. No long-lived AWS access keys in GitHub secrets.
...
Estimated AWS cost: under $5/month at simulation scale.
...
| Bug | Root Cause | Fix |
|---|---|---|
| DLM invisible to 2 buses | Boolean True != string true in DynamoDB filter | .eq(True) throughout |
...
| Prophet stale forecast | charger_kw > 0 excluded idle periods | Removed filter + resample timeline anchor to current hour |
